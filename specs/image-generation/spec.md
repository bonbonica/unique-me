...
| R3 | **`next/server`'s `after()` may not be enabled in this Next/Turbopack setup.** | **CRITICAL DEPENDENCY — verify FIRST as implementation step 1.** Without a working background-execution primitive, no images ever generate regardless of how correctly everything else is built. Build a probe (Server Action calls `after(() => { write marker row })`; assert marker appears post-return). If `after()` fires, use it. If it does not fire, use `waitUntil` from `@vercel/functions` instead. Document the chosen primitive inline in `post-service.ts` AND update this spec's File-level change summary to record which primitive was chosen. No other implementation work begins until this step lands. |
...
| `src/lib/services/post-service.ts` | Inside `generateWeekly` transaction: insert N `post_images` rows with `status="pending"`, combined imagePrompt, `attempt=1`, `source="ai"`, `selected=true`. Persist `batchImageStyle` on `weekly_batches`. After commit: call `runImageGenerationForBatch(batchId)` via `after()`. |
